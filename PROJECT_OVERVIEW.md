...
- **Accuracy is still modest (41.9%).** A real ImageNet-pretrained backbone remains the biggest lever, and remains blocked: `download.pytorch.org` was re-tested (not just assumed) and is still unreachable from this sandbox (`403 Forbidden`) — the model still trains from random initialization, which costs real accuracy. Worth simply retrying on a machine with normal internet access.
- **Class-weighted loss was tried for the Low Pressure Area / Deep Depression imbalance and honestly did NOT help — this checkpoint was not promoted.** `compute_class_weights()` in `src/train_combined.py` (sklearn-style "balanced" weighting, LPA got 28x weight) is in the codebase and easy to re-enable, but the actual retrain (30 epochs, otherwise identical) came back worse on every axis that matters: aggregate val accuracy dropped to 36.0% (from 41.9%), Low Pressure Area recall was still **0.00** (unchanged — with only 4 training examples total, upweighting the loss doesn't create signal that isn't there), and two other classes' recall regressed (Cyclonic Storm 0.23→0.00, Very Severe Cyclonic Storm 0.32→0.08) while Deep Depression improved (0.00→0.36). Net: not a real fix, just moved the imbalance around. **The real fix for Low Pressure Area is more labeled LPA images, not a loss-function trick** — worth flagging honestly rather than claiming a win. The unweighted checkpoint (`checkpoints_combined/best.pt`, 41.9% acc) remains in production.
...
- **MC-Dropout confidence is now calibration-checked (`src/calibration_check.py`) — it's moderately over-confident, not badly so.** On the same 136 held-out images: overall accuracy 41.2%, mean reported confidence 51.3%, Expected Calibration Error (ECE) = **0.103**. Reliability breakdown by confidence bin (predicted-confidence → actual accuracy in that bin): 30-40%→34%, 40-50%→35%, 50-60%→30%, 60-70%→50%, 70-80%→68%, 80-90%→50% (only 6 samples). So "70-80% confidence" predictions land close to their claimed rate, but the model runs ~10 points over-confident on average — a real, honest finding, not yet acted on (e.g. via temperature scaling) since the val set (136 images) is small enough that per-bin numbers above ~80% confidence have very few samples to trust.
...
5. ~~**Address class imbalance / accuracy.**~~ **Investigated, not adopted** — see section 4. `download.pytorch.org` re-tested, still blocked. Class-weighted loss tried and made things worse (36.0% acc vs. 41.9%, LPA recall still 0.00); code kept in `train_combined.py` but the unweighted checkpoint stays in production. Real fix needs more Low Pressure Area training images, not a loss-function change.
6. ~~**Confidence calibration check.**~~ **DONE** — see section 4. ECE=0.103, moderately over-confident; not yet wired into the dashboard as a visual (a reliability plot there is still a nice-to-have, `src/calibration_check.py` currently reports as a script).
